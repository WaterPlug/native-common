set(CONAN_MINIMUM_VERSION 2.7.0)
...
function(detect_lib_cxx LIB_CXX)
    ...
    elseif (CMAKE_CXX_COMPILER_ID MATCHES "Clang" AND NOT CMAKE_SYSTEM_NAME MATCHES "Windows")
        ...
        message(FATAL_ERROR "Unable to determine the C++ standard library for Clang.")

    elseif (CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
        # Для MSVC ничего не делаем, так как это обрабатывается отдельно
        return()

    else ()

        message(FATAL_ERROR "Unable to determine the C++ standard library. Please provide a full profile file.")
    endif ()
endfunction()
...
function(conan_install)
    ...
    execute_process(
            COMMAND ${CONAN_COMMAND} install ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/.. ${CONAN_ARGS} ${ARGN} --format=json
            RESULT_VARIABLE return_code
            OUTPUT_VARIABLE conan_stdout
            ERROR_VARIABLE conan_stderr
            ECHO_ERROR_VARIABLE    # показывать текстовый вывод независимо от результата
            WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
    )

    if (NOT "${return_code}" STREQUAL "0")
        message(FATAL_ERROR "Conan install failed with return code='${return_code}'\nError output: ${conan_stderr}")
    else ()
