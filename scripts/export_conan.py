...
"""
This script exports the native_libs_common (NLC) package and all packages
from the `conan/recipes` directory to the local Conan cache.
...
for version in versions:
    ...
    subprocess.run(["conan", "export", project_dir, "--user", "water_plug", "--channel", "native_common", "--version", version])
    for folder in os.listdir(recipes_dir):
        path = os.path.join(recipes_dir, folder)
        if os.path.isdir(path):
            subprocess.run(["conan", "export", path, "--user", "water_plug", "--channel", "native_common"], check=True)